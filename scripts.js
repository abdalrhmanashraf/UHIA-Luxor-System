...
function providerChanged() {
  var sel = document.getElementById('providerSelect');
  if(!sel.value) {
    document.getElementById('actionBtns').style.display = 'none'; return;
  }
  ...
  questions.forEach(function(q, i) {
    ...
    if(q.type === 'rating') {
      inp = '<div class="rating-wrap">' +
        q.options.map(function(op) {
          return '<button class="rating-btn" data-code="'+q.code+'"'
               + ' data-val="'+op+'" onclick="selR(this,\''+q.code+'\')">'+op+'</button>';
        }).join('') + '</div>';
      ...
      inp = '<select onchange="APP.surveyAnswers[\''+q.code+'\'] = this.value">'
          ...
          + q.options.map(function(op){ return '<option>'+op+'</option>'; }).join('')
          + '</select>';
    } else {
...
function submitSurvey() {
  ...
  callAPI('saveSurvey', {
    payload: {
      nationalId:   APP.nationalId, name:         APP.name,
      phone:        APP.phone,       category:     APP.category,
      providerCode: APP.providerCode, providerName: APP.providerName,
      answers:      APP.surveyAnswers
    }
  }, function(err, res) {
    showLoading(false);
    if(err || !res.success) { alert('خطأ في الإرسال'); btn.disabled = false; return; }
...
function toggleRecord() {
  if(!APP.mediaRecorder || APP.mediaRecorder.state === 'inactive') startRecording();
  else stopRecording();
}

function startRecording() {
  navigator.mediaDevices.getUserMedia({audio:true}).then(function(stream) {
    APP.audioBase64 = null; APP.recSeconds = 0;
    var chunks = [];
    ...
    alert('لم يُسمح بالميكروفون — استخدم رفع الملف الصوتي بدلاً منه');
  });
}
...
function submitComplaint() {
  ...
  if(!text && !APP.audioBase64) {
    alert('الرجاء كتابة الشكوى أو تسجيلها صوتياً'); return;
  }
